...
def save_source_if_changed(source_file, source_data, original_source_data):
    # In CI the generated file is not committed (gitignored), so a no-change run
    # must still write it or the deploy step will silently ship without it.
    if source_data != original_source_data or not os.path.exists(source_file):
