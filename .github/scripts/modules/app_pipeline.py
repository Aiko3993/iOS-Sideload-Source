...
def apply_bundle_id_suffix(bundle_id, app_name, base_name, is_coexist=True, github_repo=None, all_apps=None, custom_suffix=None):
    if not bundle_id or not is_coexist:
        return bundle_id, False

    if custom_suffix:
        tag = custom_suffix.strip('.')
    else:
        tag = compute_variant_tag(app_name, base_name, github_repo=github_repo, all_apps=all_apps)
...
def process_app(app_config, app_entry, client, base_name, is_coexist=True, all_apps=None):
    repo = app_config['github_repo']
    name = app_config['name']
    custom_suffix = app_config.get('bundle_id_suffix')
    ...
    tag = custom_suffix.strip('.') if custom_suffix else compute_variant_tag(name, base_name, github_repo=repo, all_apps=all_apps)
    ...
    if app_entry:
        ...
        if clean_base_for_calc == current_bundle_id:
            ...
            if is_coexist:
                ...
                full_tag = compute_variant_tag(name, base_name, github_repo=repo, all_apps=all_apps)
                if full_tag and base_for_calc.endswith(f".{full_tag}"):
                    base_for_calc = base_for_calc[:-(len(full_tag) + 1)]
            clean_base_for_calc = base_for_calc

        expected_id, _ = apply_bundle_id_suffix(clean_base_for_calc, name, base_name, is_coexist, github_repo=repo, all_apps=all_apps, custom_suffix=custom_suffix)
                    ...
                    if head_resp is None or head_resp.status_code >= 400:
                        ...
                        if repo_icons:
                            ...
                            for cand in repo_icons:
                                ...
                                total_score = q_score + score_icon_path(cand)
                                if total_score > best_icon_score:
                                    best_icon_score = total_score
                                    best_icon = cand
                            if best_icon:
            ...
            if clean_base_id == old_id:
                ...
                if is_coexist:
                    if base_id.endswith('.coexist'):
                        base_id = base_id[:-8]
                    full_tag = compute_variant_tag(name, base_name, github_repo=repo, all_apps=all_apps)
                    if full_tag and base_id.endswith(f".{full_tag}"):
                        base_id = base_id[:-(len(full_tag) + 1)]
                clean_base_id = base_id

            new_id, _ = apply_bundle_id_suffix(clean_base_id, name, base_name, is_coexist, github_repo=repo, all_apps=all_apps, custom_suffix=custom_suffix)
    ...
    try:
        ...
        target_bundle_id, needs_repackage = apply_bundle_id_suffix(bundle_id, name, base_name, is_coexist, github_repo=repo, all_apps=all_apps, custom_suffix=custom_suffix)
            ...
            if found_icon_auto:
                ...
                if icon_candidates:
                    ...
                    if best_cand:
                        ...
                        logger.info(f"Selected best quality icon for {name} (Score: {best_score}): {icon_url}")
                    else:
