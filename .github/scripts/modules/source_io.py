...
def load_existing_source(source_file, default_name, default_identifier, current_repo=None):
    ...
    # If running in CI without local source.json in git, fetch current live source from GitHub Pages CDN
    if current_repo and '/' in current_repo:
        repo_owner, repo_name = current_repo.split('/', 1)
        rel_path = source_file.replace('\\', '/')
        if rel_path.startswith('./'):
            rel_path = rel_path[2:]
        if rel_path.startswith('sources/'):
            pages_url = f"https://{repo_owner}.github.io/{repo_name}/{rel_path}"
            try:
                req = urllib.request.Request(pages_url, headers={'User-Agent': 'iOS-Sideload-Source-Builder'})
                with urllib.request.urlopen(req, timeout=10) as resp:
                    if resp.status == 200:
                        data = json.loads(resp.read().decode('utf-8'))
                        if isinstance(data, dict) and 'apps' in data:
                            logger.info(f"Loaded live baseline source with {len(data.get('apps', []))} apps from {pages_url}")
                            return data
            except Exception as e:
                logger.debug(f"Could not load live baseline from {pages_url}: {e}")
...
def generate_combined_apps_md(source_file_standard, source_file_nsfw, output_file):
    ...
    try:
        with tempfile.NamedTemporaryFile('w', dir=dir_path, delete=False, encoding='utf-8') as tmp:
            ...
            tmp.write(f"> *Last Updated: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}*\n\n")
