...
class GitHubClient:
    ...
    def get_all_releases(self, repo):
        """Fetch all releases for a repository (paginated, up to 1000). Skips cache so retention sees post-reconcile state."""
        url = f"https://api.github.com/repos/{repo}/releases"
        key = self._cache_key(url, {"per_page": 100, "max_pages": 10})
        self._paginate_cache.pop(key, None)
        return self._paginate(url, per_page=100, max_pages=10)

    def get_all_git_tags(self, repo):
        """Fetch all Git tag references for a repository."""
        url = f"https://api.github.com/repos/{repo}/git/matching-refs/tags"
        raw_refs = self._paginate(url, per_page=100, max_pages=20)
        tags = []
    ...
    def delete_git_tag(self, repo, tag):
        """Delete a Git tag reference."""
        encoded_tag = quote(tag, safe='')
        del_tag_url = f"https://api.github.com/repos/{repo}/git/refs/tags/{encoded_tag}"
        try:
            resp = self.session.delete(del_tag_url, headers=self.headers, timeout=15)
            if resp.status_code == 404:
                return True
            resp.raise_for_status()
            logger.info(f"Deleted Git tag {tag}")
            self.asset_changes.setdefault("tags_deleted", []).append({
                "repo": repo,
                "tag": tag
            })
            return True
        except Exception as e:
            logger.warning(f"Failed to delete Git tag {tag}: {e}")
            return False
...
def compute_variant_tag(app_name, base_name, github_repo=None, all_apps=None):
    if not app_name:
        return ''

    tag_parts = []
    owner = github_repo.split('/')[0].lower() if github_repo and '/' in github_repo else ''

    variant_part = ''
    if base_name and app_name.lower() != base_name.lower():
        pattern = re.compile(re.escape(base_name), re.IGNORECASE)
        variant_part = pattern.sub('', app_name).strip()

    if not variant_part:
        matches = re.findall(r'[\(\+]([^\)\+]+)\)?', app_name)
        if matches:
            variant_part = ' '.join(matches)

    words = re.findall(r'[a-zA-Z0-9]+', variant_part)

    if all_apps and github_repo:
        clean_base = re.sub(r'[\(\+].*', '', app_name).strip().lower()
        same_base_repos = {
            a.get('github_repo') for a in all_apps
            if a.get('github_repo') and re.sub(r'[\(\+].*', '', a.get('name', '')).strip().lower() == clean_base
        }
        if len(same_base_repos) > 1:
            if owner and owner != clean_base and owner not in [w.lower() for w in words]:
                tag_parts.append(owner)

    for w in words:
        w_lower = w.lower()
        if w_lower not in tag_parts:
            tag_parts.append(w_lower)
