...
def update_repo_pair(
    ...
    source_data_coex = load_existing_source(source_file_coex, source_name_coex, source_identifier_coex, current_repo=current_repo)
    source_data_orig = load_existing_source(source_file_orig, source_name_orig, source_identifier_orig, current_repo=current_repo)
    ...
    for app_config in apps:
        ...
        if clean not in base_map or len(name) < len(base_map[clean]):
            base_map[clean] = name
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for app_config in apps:
            ...
            clean = re.sub(r'[\(\+].*', '', name).strip().lower()
            base_name = base_map.get(clean, name)
            ...
            def _process_pair(cfg=app_config, entry_coex=current_entry_coex, entry_orig=current_entry_orig, base=base_name):
                entry_c, updates_c = process_app(cfg, entry_coex, client, base, True, all_apps=apps)
                entry_o, updates_o = process_app(cfg, entry_orig, client, base, False, all_apps=apps)
...
def main():
    ...
    if current_repo and client.token and not is_local_validation:
        ...
        try:
            ...
            active_release_tags = {r['tag_name'] for r in kept_releases}

            all_git_tags = client.get_all_git_tags(current_repo)
            managed_tags = [t for t in all_git_tags if t.startswith('builds-') or t.startswith('cached-') or t == 'app-artifacts']
            orphaned_tags = [t for t in managed_tags if t not in active_release_tags]

            if orphaned_tags:
                logger.info(f"Pruning {len(orphaned_tags)} orphaned Git tags...")
                for tag in orphaned_tags:
                    client.delete_git_tag(current_repo, tag)

            logger.info(f"Retention complete: {len(kept_releases)} active releases with assets, {len(orphaned_tags)} orphaned tags pruned")
        except Exception as e:
    ...
    if client.asset_changes:
        ...
        tags_deleted = client.asset_changes.get("tags_deleted", [])
        logger.info(f"Asset changes: uploaded={len(uploads)} deleted={len(deletes)} releases_deleted={len(releases_deleted)} tags_deleted={len(tags_deleted)}")
