import { TRANSLATIONS, BUTTON_CONFIG, APP_MODES, getDisplayBundleId } from './config.js';
...
export function renderApps() {
    ...
    let filtered = currentApps.filter(app =>
        ...
        (getDisplayBundleId(app) && getDisplayBundleId(app).toLowerCase().includes(query)) ||
...
function buildModalMeta(app, currentLang) {
    ...
    let html = `<div class="bg-gray-50 dark:bg-gray-800/50 p-3 rounded-xl border border-gray-100 dark:border-gray-800 ${app.minOSVersion ? '' : 'col-span-2'}">
            ...
            <div class="text-xs font-mono text-gray-600 dark:text-gray-300 break-all select-all">${getDisplayBundleId(app)}</div>
        </div>`;
