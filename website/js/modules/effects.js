import { getState } from './state.js';
import { getIcon } from './utils.js';

let eggClickCount = 0;
let eggTimer = null;

export function initCheatCodes() {
    let konamiSeq = [];
    const konamiCode = ['ArrowUp', 'ArrowUp', 'ArrowDown', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'ArrowLeft', 'ArrowRight', 'b', 'a'];
    const vimCode = ['k', 'k', 'j', 'j', 'h', 'l', 'h', 'l', 'b', 'a'];

    document.addEventListener('keydown', (e) => {
        const key = e.key.length === 1 ? e.key.toLowerCase() : e.key;
        konamiSeq.push(key);
        if (konamiSeq.length > 10) konamiSeq.shift();

        const matchKonami = konamiSeq.every((k, i) => k === konamiCode[i] || (konamiCode[i].length === 1 && k === konamiCode[i]));
        const matchVim = konamiSeq.every((k, i) => k === vimCode[i]);

        if (konamiSeq.length === 10 && (matchKonami || matchVim)) {
            playKonamiEntry();
            konamiSeq = [];
        }
    });
}

function playKonamiEntry() {
    const container = document.createElement('div');
    container.className = 'fixed inset-0 z-[10001] flex items-center justify-center pointer-events-none';
    document.body.appendChild(container);

    const text = document.createElement('div');
    text.className = 'px-4 text-center text-3xl sm:text-5xl md:text-7xl font-black text-green-500 font-mono animate-bounce drop-shadow-[0_0_15px_rgba(34,197,94,0.8)] break-words max-w-full';
    text.innerText = 'CHEAT CODE ACTIVATED';
    container.appendChild(text);

    document.body.classList.add('animate-shake');

    setTimeout(() => {
        container.remove();
        document.body.classList.remove('animate-shake');
        openDeveloperConsole();
    }, 1500);
}

export function showDeveloperConsolePrompt() {
    const searchInput = document.getElementById('search-input');
    const existingToast = document.getElementById('debug-prompt-toast');
    if (existingToast) existingToast.remove();

    const toast = document.createElement('div');
    toast.id = 'debug-prompt-toast';
    toast.className = 'absolute top-full left-0 right-0 mx-auto mt-2 z-50 flex flex-col items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl border border-green-500/30 backdrop-blur-xl animate-slide-up bg-black/90 text-green-400 font-mono w-max min-w-[280px]';
    toast.style.transition = 'opacity 0.22s ease, transform 0.22s ease';

    toast.innerHTML = `
        <div class="text-sm font-bold flex items-center gap-2">
            <span>> DETECTED_DEBUG_SEQUENCE</span>
            <span class="animate-pulse">_</span>
        </div>
        <div class="text-xs opacity-80">Initialize Developer Console?</div>
        <div class="flex gap-3 mt-2 w-full">
            <button id="dev-yes" class="flex-1 py-1.5 bg-green-500/20 hover:bg-green-500/40 border border-green-500/50 rounded text-xs font-bold transition-colors">CONFIRM</button>
            <button id="dev-no" class="flex-1 py-1.5 bg-red-500/20 hover:bg-red-500/40 border border-red-500/50 rounded text-xs font-bold transition-colors text-red-400">ABORT</button>
        </div>
    `;

    searchInput.parentElement.appendChild(toast);

    let isClosing = false;
    const cleanup = (launchConsole = false) => {
        if (isClosing) return;
        isClosing = true;
        clearTimeout(timer);
        document.removeEventListener('keydown', handleKey, true);

        toast.style.opacity = '0';
        toast.style.transform = 'translateY(-8px) scale(0.95)';

        setTimeout(() => {
            if (document.body.contains(toast)) toast.remove();
            if (launchConsole) openDeveloperConsole();
        }, 220);
    };

    const handleKey = (e) => {
        if (e.key === 'Enter') {
            e.preventDefault();
            e.stopPropagation();
            cleanup(true);
        } else if (e.key === 'Escape') {
            e.preventDefault();
            e.stopPropagation();
            cleanup(false);
        }
    };

    document.addEventListener('keydown', handleKey, true);

    const timer = setTimeout(() => {
        cleanup(false);
    }, 5000);

    document.getElementById('dev-yes').onclick = () => cleanup(true);
    document.getElementById('dev-no').onclick = () => cleanup(false);
}

export function openDeveloperConsole() {
    const { currentSource } = getState();
    const overlay = document.createElement('div');
    overlay.className = 'fixed inset-0 z-[99999] bg-black/95 text-green-500 font-mono p-4 flex flex-col animate-fade-in overflow-hidden select-none touch-none';
    overlay.style.transition = 'opacity 0.25s ease';

    document.body.style.overflow = 'hidden';
    document.body.style.touchAction = 'none';
    overlay.addEventListener('wheel', (e) => e.stopPropagation(), { passive: false });
    overlay.addEventListener('touchmove', (e) => e.stopPropagation(), { passive: false });

    const closeConsole = () => {
        overlay.style.opacity = '0';
        setTimeout(() => {
            if (document.body.contains(overlay)) overlay.remove();
            document.body.style.overflow = '';
            document.body.style.touchAction = '';
        }, 250);
    };

    const effects = [
        'emoji-rain', 'matrix-rain', 'spin-madness',
        'ascii-tux', 'retro-terminal', 'warp-speed',
        'neon-fireworks', 'retro-pong', 'element-eater'
    ];

    if (currentSource === 'nsfw') {
        const tuxIndex = effects.indexOf('ascii-tux');
        if (tuxIndex > -1) effects.splice(tuxIndex, 1);
        effects.splice(tuxIndex > -1 ? tuxIndex : effects.length, 0, 'ascii-waifu');
    }

    const devConfig = {
        duration: 5000,
        autoDismiss: true,
        gravity: 1.0,
        speed: 1.0,
        scale: 1.0
    };

    overlay.innerHTML = `
        <div class="flex justify-between items-center border-b border-green-500/30 pb-4 mb-4">
            <h2 class="text-xl font-bold flex items-center gap-2">
                <span>> DEV_CONSOLE</span>
                <span class="w-3 h-5 bg-green-500 animate-pulse inline-block"></span>
            </h2>
            <button id="dev-close" class="text-red-500 hover:text-red-400 font-bold">[EXIT]</button>
        </div>

        <div class="flex-grow overflow-y-auto custom-scrollbar">
            <div class="mb-6 border-b border-green-500/30 pb-6">
                <h3 class="text-xs font-bold text-green-400 opacity-70 mb-3 uppercase tracking-wider">> RUNTIME CONFIGURATION</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                    <div class="flex flex-col gap-1">
                        <label class="opacity-80">DURATION (MS)</label>
                        <input type="number" id="cfg-duration" value="${devConfig.duration}" class="bg-black/50 border border-green-500/30 text-green-400 px-2 py-1 rounded focus:border-green-500 outline-none">
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="opacity-80">GRAVITY (0.1 - 5.0)</label>
                        <input type="number" id="cfg-gravity" step="0.1" value="${devConfig.gravity}" class="bg-black/50 border border-green-500/30 text-green-400 px-2 py-1 rounded focus:border-green-500 outline-none">
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="opacity-80">SPEED (0.1 - 5.0)</label>
                        <input type="number" id="cfg-speed" step="0.1" value="${devConfig.speed}" class="bg-black/50 border border-green-500/30 text-green-400 px-2 py-1 rounded focus:border-green-500 outline-none">
                    </div>
                    <div class="flex flex-col gap-1">
                        <label class="opacity-80">SCALE (0.5 - 3.0)</label>
                        <input type="number" id="cfg-scale" step="0.1" value="${devConfig.scale}" class="bg-black/50 border border-green-500/30 text-green-400 px-2 py-1 rounded focus:border-green-500 outline-none">
                    </div>
                </div>
                <div class="flex items-center gap-2 mt-4">
                    <input type="checkbox" id="cfg-autodismiss" checked class="accent-green-500">
                    <label for="cfg-autodismiss" class="cursor-pointer select-none">AUTO DISMISS</label>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ${effects.map(effect => `
                    <button class="dev-effect-btn text-left p-4 border border-green-500/30 hover:bg-green-500/10 hover:border-green-500 rounded-lg transition-all group relative overflow-hidden" data-effect="${effect}">
                        <div class="absolute inset-0 bg-green-500/5 translate-y-full group-hover:translate-y-0 transition-transform duration-300"></div>
                        <div class="relative z-10 font-bold text-sm uppercase tracking-wider mb-1">./run ${effect}</div>
                        <div class="relative z-10 text-[10px] opacity-60">Execute ${effect} sequence</div>
                    </button>
                `).join('')}
            </div>

            <div class="mt-8 border-t border-green-500/30 pt-4 text-xs opacity-50">
                <p>> SYSTEM STATUS: ONLINE</p>
                <p>> MEMORY: ${Math.round(performance.memory ? performance.memory.usedJSHeapSize / 1048576 : 0)}MB USED</p>
                <p>> USER_AGENT: ${navigator.userAgent}</p>
            </div>
        </div>
    `;

    document.body.appendChild(overlay);

    document.getElementById('dev-close').onclick = closeConsole;

    const durationInput = document.getElementById('cfg-duration');
    const gravityInput = document.getElementById('cfg-gravity');
    const speedInput = document.getElementById('cfg-speed');
    const scaleInput = document.getElementById('cfg-scale');
    const dismissCheck = document.getElementById('cfg-autodismiss');

    overlay.querySelectorAll('.dev-effect-btn').forEach(btn => {
        btn.onclick = () => {
            const effect = btn.dataset.effect;
            const config = {
                duration: parseInt(durationInput.value) || 5000,
                autoDismiss: dismissCheck.checked,
                gravity: parseFloat(gravityInput.value) || 1.0,
                speed: parseFloat(speedInput.value) || 1.0,
                scale: parseFloat(scaleInput.value) || 1.0
            };

            closeConsole();
            if (effect === 'element-eater') {
                triggerElementEater(config);
            } else {
                triggerConfetti(effect, config);
            }
        };
    });
}

export function handleEasterEgg() {
    const { currentSource } = getState();
    const { isMobile } = window;
    clearTimeout(eggTimer);
    eggClickCount++;

    const pingDot = document.getElementById('status-dot-ping');
    const coreDot = document.getElementById('status-dot-core');
    const wrapper = document.getElementById('status-dot-wrapper');

    if (pingDot && coreDot && wrapper) {
        pingDot.classList.remove('bg-green-400', 'group-hover:bg-yellow-400');
        pingDot.classList.add('bg-red-500');

        coreDot.classList.remove('bg-green-500', 'group-hover:bg-yellow-500');
        coreDot.classList.add('bg-red-600');
        if (navigator.vibrate) navigator.vibrate(10);

        const limit = (typeof isMobile === 'function' && isMobile()) ? 5 : 10;

        if (eggClickCount >= limit) {
            if (navigator.vibrate) navigator.vibrate([30, 50, 30]);

            let effects = [
                'emoji-rain', 'matrix-rain', 'spin-madness',
                'ascii-tux', 'retro-terminal', 'warp-speed',
                'neon-fireworks', 'retro-pong', 'element-eater'
            ];

            if (currentSource === 'nsfw') {
                effects = effects.filter(e => e !== 'ascii-tux');
            }

            const randomEffect = effects[Math.floor(Math.random() * effects.length)];

            if (randomEffect === 'element-eater') {
                triggerElementEater();
            } else {
                playIntroTransition(() => {
                    triggerConfetti(randomEffect);
                });
            }

            eggClickCount = -999;
            return;
        }

        eggTimer = setTimeout(resetEgg, 3000);
    }
}

export function playIntroTransition(callback) {
    const footer = document.querySelector('footer');
    const wrapper = document.getElementById('status-dot-wrapper');
    const pingDot = document.getElementById('status-dot-ping');

    if (!footer || !wrapper) {
        if (callback) callback();
        return;
    }

    if (pingDot) pingDot.classList.remove('animate-ping');

    setTimeout(() => {
        footer.style.transition = 'all 0.8s ease-in-out';

        const footerBg = footer.querySelector('div');
        if (footerBg) {
            footerBg.style.transition = 'all 0.8s ease-in-out';
            footerBg.style.backgroundColor = '#ef4444';
            footerBg.style.borderColor = '#b91c1c';
            footerBg.style.boxShadow = '0 0 30px rgba(239, 68, 68, 0.6)';

            footerBg.querySelectorAll('*').forEach(el => {
                el.style.transition = 'color 0.4s ease';
                el.style.color = 'white';
            });
        }

        wrapper.style.transition = 'transform 0.4s ease';
    }, 100);

    setTimeout(() => {
        footer.style.opacity = '0';
        footer.style.transform = 'translate(-50%, 20px) scale(0.9)';
        if (callback) callback();
    }, 900);

    setTimeout(() => {
        wrapper.style.transform = 'scale(1)';
        if (pingDot) pingDot.classList.add('animate-ping');
        footer.style.transform = '';

        requestAnimationFrame(() => {
            footer.style.opacity = '1';
            const footerBg = footer.querySelector('div');
            if (footerBg) {
                footerBg.style.backgroundColor = '';
                footerBg.style.borderColor = '';
                footerBg.style.boxShadow = '';
                footerBg.querySelectorAll('*').forEach(el => el.style.color = '');
            }
            resetEgg();
        });
    }, 5500);
}

export function triggerElementEater(config = {}, onComplete = null) {
    const cfg = {
        duration: 3500,
        autoDismiss: true,
        speed: 1.0,
        ...config
    };

    const origOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';

    const selectors = [
        'footer',
        '.app-card',
        '#empty-state',
        '#category-bar',
        '#header-bar'
    ];

    const rawElements = Array.from(document.querySelectorAll(selectors.join(',')))
        .filter(el => {
            const rect = el.getBoundingClientRect();
            return rect.width > 15 && rect.height > 10;
        });

    const targetElements = rawElements.filter(el => {
        return !rawElements.some(parent => parent !== el && parent.contains(el) && !parent.matches('#header-bar, footer, #category-bar'));
    });

    if (targetElements.length === 0) {
        document.body.style.overflow = origOverflow;
        if (onComplete) onComplete();
        return;
    }

    const savedStyles = new Map();
    targetElements.forEach(el => {
        const rect = el.getBoundingClientRect();
        savedStyles.set(el, {
            rectTop: rect.top + window.scrollY,
            transition: el.style.transition,
            transform: el.style.transform,
            opacity: el.style.opacity,
            filter: el.style.filter,
            pointerEvents: el.style.pointerEvents
        });
    });

    const getBaseTransform = (el) => {
        return (el.classList.contains('-translate-x-1/2') || el.matches('footer, #toast-container, #pull-refresh-indicator'))
            ? 'translateX(-50%) '
            : '';
    };

    const sortedDevour = [...targetElements].sort((a, b) => {
        return (savedStyles.get(b).rectTop) - (savedStyles.get(a).rectTop);
    });

    const stepDelay = Math.max(18, Math.min(38, (850 / sortedDevour.length) / cfg.speed));

    sortedDevour.forEach((el, index) => {
        setTimeout(() => {
            const baseT = getBaseTransform(el);
            el.style.transition = 'transform 0.38s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.38s ease, filter 0.38s ease';
            el.style.transform = `${baseT}translateY(16px) scale(0.9)`;
            el.style.opacity = '0';
            el.style.filter = 'blur(6px)';
            el.style.pointerEvents = 'none';
        }, index * stepDelay);
    });

    const totalDevourTime = sortedDevour.length * stepDelay + 380;
    const turnaroundDelay = totalDevourTime + 150;

    setTimeout(() => {
        const sortedRestore = [...targetElements].sort((a, b) => {
            return (savedStyles.get(a).rectTop) - (savedStyles.get(b).rectTop);
        });

        sortedRestore.forEach((el, index) => {
            setTimeout(() => {
                const baseT = getBaseTransform(el);
                el.style.transition = 'transform 0.46s cubic-bezier(0.16, 1, 0.3, 1), opacity 0.46s ease, filter 0.46s ease';
                el.style.transform = `${baseT}translateY(0) scale(1)`;
                el.style.opacity = '1';
                el.style.filter = 'none';
                el.style.pointerEvents = savedStyles.get(el).pointerEvents || '';
            }, index * stepDelay);
        });

        const totalRestoreTime = sortedRestore.length * stepDelay + 500;

        setTimeout(() => {
            sortedRestore.forEach(el => {
                const saved = savedStyles.get(el);
                if (saved) {
                    el.style.transition = saved.transition;
                    el.style.transform = saved.transform;
                    el.style.opacity = saved.opacity;
                    el.style.filter = saved.filter;
                    el.style.pointerEvents = saved.pointerEvents;
                }
            });

            document.body.style.overflow = origOverflow;
            resetEgg();
            if (onComplete) onComplete();
        }, totalRestoreTime);

    }, turnaroundDelay);
}

export function resetEgg() {
    eggClickCount = 0;
    const pingDot = document.getElementById('status-dot-ping');
    const coreDot = document.getElementById('status-dot-core');
    const wrapper = document.getElementById('status-dot-wrapper');

    if (pingDot && coreDot && wrapper) {
        pingDot.classList.remove('bg-red-500');
        pingDot.classList.add('bg-green-400', 'group-hover:bg-yellow-400');
        pingDot.style.animationDuration = '';

        coreDot.classList.remove('bg-red-600');
        coreDot.classList.add('bg-green-500', 'group-hover:bg-yellow-500');

        wrapper.style.transform = '';
        wrapper.style.animation = '';
        wrapper.classList.remove('opacity-0', 'scale-0');
    }
}

export function triggerConfetti(forcedEffect = null, config = {}) {
    const { isMobile } = window;

    const cfg = {
        duration: 5000,
        autoDismiss: true,
        gravity: 1.0,
        speed: 1.0,
        scale: 1.0,
        ...config
    };

    if (forcedEffect === 'element-eater') {
        triggerElementEater(cfg);
        return;
    }

    const effects = [
        'emoji-rain', 'matrix-rain', 'spin-madness',
        'ascii-tux', 'retro-terminal', 'warp-speed',
        'neon-fireworks', 'retro-pong', 'element-eater'
    ];

    const effect = forcedEffect || effects[Math.floor(Math.random() * effects.length)];

    if (effect === 'element-eater') {
        triggerElementEater(cfg);
        return;
    }

    const container = document.createElement('div');
    container.style.position = 'fixed';
    container.style.top = '0';
    container.style.left = '0';
    container.style.width = '100%';
    container.style.height = '100%';
    container.style.pointerEvents = 'auto';
    container.style.zIndex = '9999';
    container.style.overflow = 'hidden';
    container.style.opacity = '0';
    container.style.transition = 'opacity 0.4s ease-in-out';
    container.style.touchAction = 'none';
    container.style.userSelect = 'none';
    container.style.webkitUserSelect = 'none';

    document.body.style.overflow = 'hidden';
    document.body.style.touchAction = 'none';
    container.addEventListener('wheel', (e) => e.stopPropagation(), { passive: false });
    container.addEventListener('touchmove', (e) => e.stopPropagation(), { passive: false });

    document.body.appendChild(container);

    requestAnimationFrame(() => {
        container.style.opacity = '1';
    });

    const fadeOutAndRemove = (delay) => {
        if (delay > 0 && !cfg.autoDismiss) return;

        setTimeout(() => {
            container.style.opacity = '0';
            setTimeout(() => {
                if (document.body.contains(container)) document.body.removeChild(container);
                document.body.style.overflow = '';
                document.body.style.touchAction = '';
                document.body.style.transform = '';
            }, 400);
        }, delay);
    };

    const interactiveEffects = ['retro-pong', 'neon-fireworks', 'warp-speed', 'matrix-rain', 'ascii-tux', 'ascii-waifu', 'retro-terminal'];
    if (interactiveEffects.includes(effect)) {
        const closeBtn = document.createElement('button');
        closeBtn.setAttribute('aria-label', 'Close Easter Egg');
        closeBtn.innerHTML = getIcon('close', 'w-6 h-6 group-hover:rotate-90 transition-transform duration-200');
        closeBtn.className = 'fixed top-5 right-5 z-[99999] p-3 bg-black/60 hover:bg-black/90 active:scale-90 border border-white/30 hover:border-white/70 backdrop-blur-2xl rounded-full text-white/90 hover:text-white transition-all duration-300 shadow-[0_8px_30px_rgba(0,0,0,0.6)] flex items-center justify-center cursor-pointer pointer-events-auto select-none group';
        closeBtn.addEventListener('pointerdown', (e) => e.stopPropagation());
        closeBtn.addEventListener('mousedown', (e) => e.stopPropagation());
        closeBtn.addEventListener('touchstart', (e) => e.stopPropagation(), { passive: false });
        closeBtn.onclick = (e) => {
            e.stopPropagation();
            fadeOutAndRemove(0);
        };
        container.appendChild(closeBtn);
    }

    if (effect === 'emoji-rain') {
        const baseEmojis = [
            '💥', '📱', '🎉', '🍎', '💻', '🚀', '💊', '👻', '🧱', '🔥', '✨',
            '💿', '💾', '🕹️', '👾', '🤖', '👽', '🦄', '🌈', '🍕', '🍔', '🍺',
            '💡', '📷', '🔋', '🔌', '📡', '🔭', '🎁', '🎈', '🎊', '🏆'
        ];

        const twemojiBase = 'https://cdn.jsdelivr.net/gh/twitter/twemoji@latest/assets/svg/';
        const getHex = (char) => char.codePointAt(0).toString(16);

        for (let i = 0; i < 60; i++) {
            const el = document.createElement('div');
            const char = baseEmojis[Math.floor(Math.random() * baseEmojis.length)];

            if (Math.random() < 0.3) {
                const img = document.createElement('img');
                img.src = `${twemojiBase}${getHex(char)}.svg`;
                img.style.width = '100%';
                img.style.height = '100%';
                el.appendChild(img);
            } else {
                el.innerText = char;
                const fonts = [
                    '"Apple Color Emoji", "Segoe UI Emoji", "Noto Color Emoji", sans-serif',
                    '"Noto Color Emoji", sans-serif',
                    '"Segoe UI Emoji", sans-serif'
                ];
                el.style.fontFamily = fonts[Math.floor(Math.random() * fonts.length)];
            }

            el.style.position = 'absolute';
            el.style.left = Math.random() * 100 + 'vw';
            el.style.top = '-50px';
            const size = (Math.random() * 20 + 20) * cfg.scale;
            el.style.fontSize = size + 'px';
            el.style.width = size + 'px';
            el.style.height = size + 'px';

            const duration = (Math.random() * 2 + 1) / cfg.gravity;
            el.style.animation = `fall-down ${duration}s linear forwards`;
            el.style.animationDelay = Math.random() * 0.5 + 's';
            container.appendChild(el);
        }

        fadeOutAndRemove(cfg.duration);
    } else if (effect === 'matrix-rain') {
        container.style.pointerEvents = 'auto';
        const canvas = document.createElement('canvas');
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        container.style.background = 'black';
        container.style.opacity = '0.9';
        container.appendChild(canvas);

        const ctx = canvas.getContext('2d');
        const chars = "01日ﾊﾐﾋｰｳｼﾅﾓﾆｻﾜﾂｵﾘｱﾎﾃﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍ12345789:・.=\"*+-<>¦｜";

        let columns = Math.ceil(canvas.width / 20);
        let drops = Array(columns).fill(1);

        const onResize = () => {
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            const newColumns = Math.ceil(canvas.width / 20);
            if (newColumns > columns) {
                const added = new Array(newColumns - columns).fill(1);
                drops = drops.concat(added);
            } else if (newColumns < columns) {
                drops = drops.slice(0, newColumns);
            }
            columns = newColumns;
        };
        window.addEventListener('resize', onResize);

        function drawMatrix() {
            if (canvas.width !== window.innerWidth || canvas.height !== window.innerHeight) {
                onResize();
            }

            ctx.fillStyle = 'rgba(0, 0, 0, 0.05)';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            ctx.fillStyle = '#0F0';
            ctx.font = '15px monospace';
            for (let i = 0; i < drops.length; i++) {
                const text = chars[Math.floor(Math.random() * chars.length)];
                ctx.fillText(text, i * 20, drops[i] * 20);
                if (drops[i] * 20 > canvas.height && Math.random() > 0.975) {
                    drops[i] = 0;
                }
                drops[i]++;
            }
        }
        const interval = setInterval(drawMatrix, 50 / cfg.speed);

        if (cfg.autoDismiss) {
            setTimeout(() => {
                clearInterval(interval);
                window.removeEventListener('resize', onResize);
                fadeOutAndRemove(0);
            }, cfg.duration);
        }
    } else if (effect === 'spin-madness') {
        document.body.style.transition = 'transform 1s ease-in-out';
        document.body.style.transform = 'rotate(360deg)';
        setTimeout(() => { document.body.style.transform = ''; }, 1000);
        const colors = ['#f00', '#0f0', '#00f', '#ff0', '#0ff', '#f0f'];
        for (let i = 0; i < 50; i++) {
            const el = document.createElement('div');
            el.style.position = 'absolute';
            el.style.width = '10px';
            el.style.height = '10px';
            el.style.backgroundColor = colors[Math.floor(Math.random() * colors.length)];
            el.style.left = '50%';
            el.style.top = '50%';
            const angle = Math.random() * Math.PI * 2;
            const velocity = Math.random() * 500 + 200;
            const tx = Math.cos(angle) * velocity;
            const ty = Math.sin(angle) * velocity;
            el.style.transition = `all ${1 / cfg.speed}s ease-out`;
            container.appendChild(el);
            requestAnimationFrame(() => {
                el.style.transform = `translate(${tx}px, ${ty}px) rotate(${Math.random() * 720}deg)`;
                el.style.opacity = '0';
            });
        }
        fadeOutAndRemove(Math.min(cfg.duration, 4000));
    } else if (effect === 'ascii-waifu') {
        container.style.pointerEvents = 'auto';
        container.style.backgroundColor = 'rgba(0, 0, 0, 0.9)';
        container.style.display = 'flex';
        container.style.alignItems = 'center';
        container.style.justifyContent = 'center';
        container.style.zIndex = '10000';

        const terminal = document.createElement('div');
        terminal.className = 'w-full max-w-2xl bg-black border border-green-500 font-mono text-green-500 p-6 rounded shadow-[0_0_30px_rgba(34,197,94,0.3)] flex flex-col gap-4 m-4 animate-scale-up';
        terminal.innerHTML = `
            <div class="border-b border-green-500/50 pb-2 mb-2 flex justify-between items-center">
                <span class="font-bold text-lg">> ART_COMPILER_V1.0</span>
                <span class="animate-pulse">_</span>
            </div>

            <div class="flex flex-col gap-2">
                <label class="text-xs font-bold opacity-70">>> INPUT_ASCII_SEQUENCE</label>
                <textarea id="waifu-art" rows="10" class="w-full bg-black/50 border border-green-500/30 text-xs p-2 focus:border-green-500 outline-none text-green-400 custom-scrollbar" placeholder="Paste ASCII Art Here..."></textarea>
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="flex flex-col gap-2">
                    <label class="text-xs font-bold opacity-70">>> ASPECT_RATIO (W:H)</label>
                    <input type="text" id="waifu-ratio" value="1:1" class="w-full bg-black/50 border border-green-500/30 text-xs p-2 focus:border-green-500 outline-none text-green-400" placeholder="e.g. 3:4">
                </div>
                <div class="flex flex-col gap-2">
                    <label class="text-xs font-bold opacity-70">>> COLOR_HEX</label>
                    <input type="text" id="waifu-color" value="#ff69b4" class="w-full bg-black/50 border border-green-500/30 text-xs p-2 focus:border-green-500 outline-none text-green-400" placeholder="#RRGGBB">
                </div>
            </div>

            <div class="pt-4 flex gap-3">
                <button id="waifu-submit" class="flex-1 py-2 bg-green-500/20 hover:bg-green-500/40 border border-green-500 text-green-400 font-bold uppercase tracking-wider transition-all">
                    > COMPILE & RENDER
                </button>
                <button id="waifu-cancel" class="px-4 py-2 bg-red-500/20 hover:bg-red-500/40 border border-red-500 text-red-400 font-bold uppercase tracking-wider transition-all">
                    ABORT
                </button>
            </div>
        `;
        container.appendChild(terminal);

        const cleanUpInput = () => {
            terminal.remove();
        };

        terminal.querySelector('#waifu-cancel').onclick = () => {
            fadeOutAndRemove(0);
        };

        terminal.querySelector('#waifu-submit').onclick = (e) => {
            e.stopPropagation();
            const art = terminal.querySelector('#waifu-art').value;
            const ratio = terminal.querySelector('#waifu-ratio').value;
            const color = terminal.querySelector('#waifu-color').value;

            if (!art.trim()) {
                alert("ERROR: NULL_INPUT_DETECTED");
                return;
            }

            cleanUpInput();
            renderWaifu(art, ratio, color);
        };

        terminal.onclick = (e) => {
            e.stopPropagation();
        };

        const renderWaifu = (artStr, ratioStr, colorStr) => {
            container.style.backgroundColor = '#000000';

            const pre = document.createElement('pre');
            pre.style.position = 'absolute';
            pre.style.top = '50%';
            pre.style.left = '50%';
            pre.style.transformOrigin = 'center center';
            pre.style.fontSize = (10 * cfg.scale) + 'px';
            pre.style.lineHeight = (10 * cfg.scale) + 'px';
            pre.style.fontFamily = 'monospace';
            pre.style.transition = 'opacity 1s ease-out';
            pre.style.opacity = '0';
            pre.style.userSelect = 'none';
            pre.style.color = colorStr;
            pre.innerText = artStr;

            container.appendChild(pre);

            const scaleArt = () => {
                const lines = artStr.split('\n');
                const artHeight = lines.length;
                const artWidth = lines.reduce((max, line) => Math.max(max, line.length), 0);

                const screenW = window.innerWidth * 0.9;
                const screenH = window.innerHeight * 0.9;

                const charW = 6;
                const charH = 10;

                let scaleYCorrection = 1;
                if (ratioStr) {
                    const parts = ratioStr.split(':');
                    if (parts.length === 2) {
                        const rW = parseFloat(parts[0]);
                        const rH = parseFloat(parts[1]);
                        if (rW && rH) {
                            const targetRatio = rW / rH;
                            const currentRatio = (artWidth * charW) / (artHeight * charH);
                            scaleYCorrection = currentRatio / targetRatio;
                        }
                    }
                }

                const scaleX = screenW / (artWidth * charW);
                const scaleY = screenH / (artHeight * charH * scaleYCorrection);
                const scale = Math.min(scaleX, scaleY);

                pre.style.transform = `translate(-50%, -50%) scale(${scale}, ${scale * scaleYCorrection})`;
            };

            requestAnimationFrame(() => {
                scaleArt();
                pre.style.opacity = '1';
            });

            window.addEventListener('resize', scaleArt);

            const cleanup = () => {
                window.removeEventListener('resize', scaleArt);
                fadeOutAndRemove(0);
            };

            if (cfg.autoDismiss) {
                setTimeout(cleanup, cfg.duration);
            }

            container.onclick = cleanup;

            const closeBtn = document.createElement('button');
            closeBtn.innerHTML = getIcon('close', 'w-8 h-8');
            closeBtn.className = 'fixed top-8 right-8 z-[10001] p-3 bg-white/10 hover:bg-white/30 backdrop-blur-md rounded-full text-white transition-all duration-500 opacity-0 pointer-events-none';

            setTimeout(() => {
                closeBtn.classList.remove('opacity-0', 'pointer-events-none');
            }, 2000);

            const btnContainer = document.createElement('div');
            btnContainer.className = 'absolute inset-0 pointer-events-none group';
            btnContainer.appendChild(closeBtn);
            container.appendChild(btnContainer);

            closeBtn.style.pointerEvents = 'auto';
            closeBtn.onclick = (e) => {
                e.stopPropagation();
                cleanup();
            };
        };

    } else if (effect === 'ascii-tux') {
        const pre = document.createElement('pre');
        pre.style.position = 'absolute';
        pre.style.top = '50%';
        pre.style.left = '50%';
        pre.style.transform = 'translate(-50%, -50%) scale(0.1)';
        pre.style.fontSize = '12px';
        pre.style.lineHeight = '12px';
        pre.style.color = '#000';
        pre.style.fontFamily = 'monospace';
        pre.style.transition = 'all 0.5s ease-out';
        pre.style.opacity = '0';
        if (document.documentElement.classList.contains('dark')) pre.style.color = '#fff';

        let baseScale = 1.5;
        const updateScale = () => {
            const minDim = Math.min(window.innerWidth, window.innerHeight);
            baseScale = Math.max(1.5, (minDim / 400) * 1.5) * cfg.scale;
        };
        window.addEventListener('resize', updateScale);
        updateScale();

        const arts = {
            tux: {
                frames: [`
         _nnnn_
        dGGGGMMb
       @p~qp~~qMb
       M|@||@) M|
       @,----.JM|
      JS^\\__/  qKL
     dZP        qKRb
    dZP          qKKb
   fZP            SMMb
   HZM            MMMM
   FqM            MMMM
 __| ".        |\\dS"qML
 |    \`.       | \`' \\Zq
_)      \\.___.,|     .'
\\____   )MMMMMP|   .'
     \`-'       \`--'
`],
                animator: (elapsed, baseArt) => {
                    const closedEyes = baseArt.replace('M|@||@)', 'M|-||-)');
                    return (elapsed % 3000 < 150) ? closedEyes : baseArt;
                }
            },
            dragon: {
                frames: [`
                \\||/
                |  @___oo
      /\\  /\\   / (__,,,,|
     ) /^\\) ^\\/ _)
     )   /^\\/   _)
     )   _ /  / _)
 /\\  )/\\/ ||  | )_)
<  >      |(,,) )__)
 ||      /    \\)___)\\
 | \\____(      )___) )___
  \\______(_______;;; __;;;
`],
                animator: (elapsed, baseArt) => {
                    if (elapsed % 2000 < 1000) {
                        return baseArt.replace('o', 'O').replace('o', 'O');
                    }
                    return baseArt.replace('oo', 'oo  🔥🔥🔥');
                }
            },
            amogus: {
                frames: [`
      . 　　　。　　　　•　 　ﾟ　　。 　　.

　　　.　　　 　　.　　　　　。　　 。　.

.　　 。　　　　　 ඞ 。 . 　　 • 　　　　•

　　ﾟ　　 Blue was An Impostor.　 。　.

　　'　　　 1 Impostor remains 　 　　。

　　ﾟ　　　.　　　. ,　　　　.　 .
`],
                animator: (elapsed, baseArt) => baseArt
            },
            rick: {
                frames: [`
      .---.
     / o o \\
    (   ^   )
     \\  -  /
      |||||
     /|___|\\
    / |   | \\
      |   |
     /     \\
    |       |
`],
                animator: (elapsed, baseArt) => {
                    const lyrics = [
                        "Never gonna give you up",
                        "Never gonna let you down",
                        "Never gonna run around",
                        "and desert you"
                    ];
                    const index = Math.floor(elapsed / 1500) % lyrics.length;

                    let art = baseArt;
                    if (Math.floor(elapsed / 250) % 2 === 0) {
                        art = art.replace('/|___|\\', '\\|___|/').replace('(   ^   )', '(   >   )');
                    } else {
                        art = art.replace('(   ^   )', '(   <   )');
                    }

                    return art + "\n\n" + lyrics[index];
                }
            }
        };

        let selectedKey;
        if (effect === 'ascii-waifu') {
            selectedKey = 'waifu';
        } else {
            const artKeys = Object.keys(arts).filter(k => k !== 'waifu');
            selectedKey = artKeys[Math.floor(Math.random() * artKeys.length)];
        }

        const selectedArt = arts[selectedKey];
        const baseFrame = selectedArt.frames[0];

        pre.innerText = baseFrame;
        container.appendChild(pre);

        requestAnimationFrame(() => {
            const xOffset = '15px';
            pre.style.transform = `translate(calc(-50% + ${xOffset}), -50%) scale(${baseScale})`;
            pre.style.opacity = '1';

            const onMouseMove = (e) => {
                let clientX, clientY;
                if (e.touches && e.touches.length > 0) {
                    clientX = e.touches[0].clientX;
                    clientY = e.touches[0].clientY;
                } else {
                    clientX = e.clientX;
                    clientY = e.clientY;
                }

                const centerX = window.innerWidth / 2;
                const centerY = window.innerHeight / 2;
                const dx = clientX - centerX;
                const dy = clientY - centerY;
                const rotation = Math.max(-30, Math.min(30, dx / 20));
                const lean = Math.max(-0.2, Math.min(0.2, dy / 500));
                pre.style.transform = `translate(calc(-50% + ${xOffset}), -50%) scale(${baseScale + lean}) rotate(${rotation}deg)`;
            };
            document.addEventListener('mousemove', onMouseMove);
            document.addEventListener('touchmove', onMouseMove, { passive: false });

            let startTime = Date.now();
            const waddle = setInterval(() => {
                const elapsed = Date.now() - startTime;
                if (selectedArt.animator) {
                    pre.innerText = selectedArt.animator(elapsed * cfg.speed, baseFrame);
                }
            }, 50);

            if (cfg.autoDismiss) {
                setTimeout(() => {
                    clearInterval(waddle);
                    window.removeEventListener('resize', updateScale);
                    document.removeEventListener('mousemove', onMouseMove);
                    document.removeEventListener('touchmove', onMouseMove);
                    pre.style.transition = 'all 0.5s ease-in';
                    pre.style.transform = 'translate(-50%, -50%) scale(0) rotate(720deg)';
                    pre.style.opacity = '0';
                    fadeOutAndRemove(500);
                }, cfg.duration);
            }
        });
    } else if (effect === 'retro-terminal') {
        container.style.background = '#000';
        container.style.color = '#0f0';
        container.style.fontFamily = 'monospace';
        container.style.padding = '20px';
        container.style.display = 'flex';
        container.style.alignItems = 'center';
        container.style.justifyContent = 'center';
        container.style.fontSize = (24 * cfg.scale) + 'px';
        container.innerText = '> SYSTEM COMPROMISED...\n> REBOOTING KERNEL...\n> LOADING...';

        setTimeout(() => container.innerText += '\n> ACCESS GRANTED.', 1000 / cfg.speed);
        setTimeout(() => container.innerText += '\n> JUST KIDDING :P', 2000 / cfg.speed);
        fadeOutAndRemove(cfg.duration);
    } else if (effect === 'warp-speed') {
        container.style.pointerEvents = 'auto';
        const canvas = document.createElement('canvas');
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        container.style.background = 'black';
        container.appendChild(canvas);
        const ctx = canvas.getContext('2d');

        let cx = canvas.width / 2;
        let cy = canvas.height / 2;

        const onResize = () => {
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            cx = canvas.width / 2;
            cy = canvas.height / 2;
        };
        window.addEventListener('resize', onResize);

        class Star {
            constructor() {
                this.reset();
            }
            reset() {
                this.x = (Math.random() - 0.5) * canvas.width * 2;
                this.y = (Math.random() - 0.5) * canvas.height * 2;
                this.z = Math.random() * 2000 + 500;
                this.pz = this.z;
                this.color = `hsl(${Math.random() * 60 + 200}, 100%, 80%)`;
            }
            update(speed) {
                this.z -= speed;
                if (this.z < 1) {
                    this.reset();
                    this.z = 2000;
                    this.pz = this.z;
                }
            }
            draw() {
                const x = cx + (this.x / this.z) * 1000;
                const y = cy + (this.y / this.z) * 1000;
                const px = cx + (this.x / this.pz) * 1000;
                const py = cy + (this.y / this.pz) * 1000;
                this.pz = this.z;
                if (x < 0 || x > canvas.width || y < 0 || y > canvas.height) return;
                const size = (1 - this.z / 2000) * 4;
                const alpha = (1 - this.z / 2000);
                ctx.beginPath();
                ctx.strokeStyle = this.color;
                ctx.lineWidth = size;
                ctx.globalAlpha = alpha;
                ctx.moveTo(px, py);
                ctx.lineTo(x, y);
                ctx.stroke();
                ctx.globalAlpha = 1;
            }
        }

        const stars = [];
        for (let i = 0; i < 800; i++) stars.push(new Star());

        let speed = 2;
        let targetSpeed = 2;
        let isAccelerating = false;
        let isStopping = false;
        let stopTimer = null;

        const startAccel = (e) => {
            e.preventDefault();
            if (isStopping) {
                isStopping = false;
                if (stopTimer) {
                    clearTimeout(stopTimer);
                    stopTimer = null;
                }
            }
            isAccelerating = true;
            targetSpeed = 50;
        };

        const stopAccel = (e) => {
            e.preventDefault();
            isAccelerating = false;
            targetSpeed = 2;

            if (stopTimer) clearTimeout(stopTimer);
            stopTimer = setTimeout(() => {
                isStopping = true;
                targetSpeed = 0;
            }, 2000);
        };

        container.addEventListener('mousedown', startAccel);
        container.addEventListener('touchstart', startAccel, { passive: false });
        container.addEventListener('mouseup', stopAccel);
        container.addEventListener('touchend', stopAccel);
        container.addEventListener('mouseleave', stopAccel);

        function animate() {
            if (canvas.width !== window.innerWidth || canvas.height !== window.innerHeight) {
                onResize();
            }

            ctx.fillStyle = 'rgba(0, 0, 0, 0.3)';
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            speed += (targetSpeed - speed) * 0.05;

            if (isStopping && speed < 0.1) {
                window.removeEventListener('resize', onResize);
                fadeOutAndRemove(0);
                return;
            }

            stars.forEach(star => {
                star.update(speed * cfg.speed);
                star.draw();
            });
            if (document.body.contains(container)) requestAnimationFrame(animate);
        }

        const hint = document.createElement('div');
        hint.className = "absolute bottom-10 left-0 right-0 text-center text-cyan-400 font-mono text-sm opacity-50 animate-pulse pointer-events-none select-none";
        hint.innerText = "HOLD TO WARP // RELEASE TO EXIT";
        container.appendChild(hint);

        animate();

        if (cfg.autoDismiss && cfg.duration > 0) {
            setTimeout(() => {
                isStopping = true;
                targetSpeed = 0;
            }, cfg.duration);
        }

        return;
    } else if (effect === 'neon-fireworks') {
        container.style.pointerEvents = 'auto';
        container.style.background = 'radial-gradient(circle at center, rgba(10, 15, 30, 0.96) 0%, rgba(2, 4, 10, 0.99) 100%)';
        document.body.style.overflow = 'hidden';
        document.body.style.touchAction = 'none';

        const canvas = document.createElement('canvas');
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        canvas.style.position = 'absolute';
        canvas.style.inset = '0';
        canvas.style.cursor = 'crosshair';
        container.appendChild(canvas);
        const ctx = canvas.getContext('2d');

        const resize = () => {
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
        };
        window.addEventListener('resize', resize);

        const palettes = [
            ['#38bdf8', '#0284c7', '#7dd3fc', '#ffffff'],
            ['#f43f5e', '#fb7185', '#fda4af', '#ffffff'],
            ['#a855f7', '#c084fc', '#e879f9', '#ffffff'],
            ['#fbbf24', '#f59e0b', '#fef08a', '#ffffff'],
            ['#10b981', '#34d399', '#6ee7b7', '#ffffff'],
            ['#ec4899', '#38bdf8', '#fbbf24', '#a855f7']
        ];

        const rockets = [];
        const particles = [];
        const shockwaves = [];

        const isCloseButtonArea = (x, y) => x > window.innerWidth - 75 && y < 75;

        const launchRocket = (targetX, targetY) => {
            const startX = Math.random() * (canvas.width * 0.8) + (canvas.width * 0.1);
            const startY = canvas.height;
            const distance = Math.hypot(targetX - startX, targetY - startY);
            const speed = Math.max(12, Math.min(22, distance * 0.028)) * cfg.speed;
            const angle = Math.atan2(targetY - startY, targetX - startX);
            const palette = palettes[Math.floor(Math.random() * palettes.length)];

            rockets.push({
                x: startX,
                y: startY,
                targetX: targetX,
                targetY: targetY,
                vx: Math.cos(angle) * speed,
                vy: Math.sin(angle) * speed,
                color: palette[0],
                palette: palette,
                trail: []
            });
        };

        const explodeRocket = (x, y, palette) => {
            const particleCount = typeof isMobile === 'function' && isMobile() ? 65 : 110;
            const burstType = Math.random() < 0.35 ? 'ring' : 'peony';

            shockwaves.push({
                x: x,
                y: y,
                radius: 4,
                maxRadius: Math.random() * 30 + 45,
                color: palette[0],
                alpha: 0.9
            });

            for (let i = 0; i < particleCount; i++) {
                let angle, speed;
                if (burstType === 'ring') {
                    angle = (i / particleCount) * Math.PI * 2 + (Math.random() - 0.5) * 0.2;
                    speed = (Math.random() * 1.5 + 4.5) * cfg.speed;
                } else {
                    angle = Math.random() * Math.PI * 2;
                    speed = (Math.random() * 6.5 + 1.2) * cfg.speed;
                }

                const color = palette[Math.floor(Math.random() * palette.length)];
                particles.push({
                    x: x,
                    y: y,
                    vx: Math.cos(angle) * speed,
                    vy: Math.sin(angle) * speed,
                    color: color,
                    alpha: 1.0,
                    decay: Math.random() * 0.016 + 0.011,
                    size: Math.random() * 2.8 + 1.2,
                    flicker: Math.random() < 0.4
                });
            }

            if (navigator.vibrate) navigator.vibrate(14);
        };

        let isPointerActive = false;
        let lastLaunchTime = 0;

        const handlePointerAction = (e) => {
            const p = e.touches && e.touches.length > 0 ? e.touches[0] : e;
            if (isCloseButtonArea(p.clientX, p.clientY)) return;

            const now = Date.now();
            if (now - lastLaunchTime > 110) {
                lastLaunchTime = now;
                launchRocket(p.clientX, p.clientY);
            }
        };

        const onPointerDown = (e) => {
            isPointerActive = true;
            handlePointerAction(e);
        };

        const onPointerMove = (e) => {
            if (isPointerActive) {
                handlePointerAction(e);
            }
        };

        const onPointerUp = () => {
            isPointerActive = false;
        };

        window.addEventListener('pointerdown', onPointerDown);
        window.addEventListener('pointermove', onPointerMove);
        window.addEventListener('pointerup', onPointerUp);
        window.addEventListener('touchstart', onPointerDown, { passive: false });
        window.addEventListener('touchmove', onPointerMove, { passive: false });
        window.addEventListener('touchend', onPointerUp);

        let autoLaunchTimer = 0;

        const animate = () => {
            if (!document.body.contains(container)) return;

            ctx.globalCompositeOperation = 'source-over';
            ctx.fillStyle = 'rgba(2, 4, 12, 0.2)';
            ctx.fillRect(0, 0, canvas.width, canvas.height);
            ctx.globalCompositeOperation = 'lighter';

            autoLaunchTimer++;
            if (autoLaunchTimer > 45) {
                autoLaunchTimer = 0;
                const tx = Math.random() * (canvas.width * 0.75) + (canvas.width * 0.125);
                const ty = Math.random() * (canvas.height * 0.5) + (canvas.height * 0.1);
                launchRocket(tx, ty);
            }

            for (let i = shockwaves.length - 1; i >= 0; i--) {
                const sw = shockwaves[i];
                sw.radius += 3.5 * cfg.speed;
                sw.alpha *= 0.91;
                if (sw.alpha <= 0.02 || sw.radius >= sw.maxRadius) {
                    shockwaves.splice(i, 1);
                    continue;
                }

                ctx.save();
                ctx.strokeStyle = sw.color;
                ctx.lineWidth = 2;
                ctx.globalAlpha = sw.alpha;
                ctx.shadowColor = sw.color;
                ctx.shadowBlur = 12;
                ctx.beginPath();
                ctx.arc(sw.x, sw.y, sw.radius, 0, Math.PI * 2);
                ctx.stroke();
                ctx.restore();
            }

            for (let i = rockets.length - 1; i >= 0; i--) {
                const r = rockets[i];
                r.trail.push({ x: r.x, y: r.y });
                if (r.trail.length > 6) r.trail.shift();

                r.x += r.vx;
                r.y += r.vy;

                for (let t = 0; t < r.trail.length; t++) {
                    const tr = r.trail[t];
                    const alpha = ((t + 1) / r.trail.length) * 0.5;
                    ctx.save();
                    ctx.fillStyle = r.color;
                    ctx.globalAlpha = alpha;
                    ctx.beginPath();
                    ctx.arc(tr.x, tr.y, 1.8, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.restore();
                }

                ctx.save();
                ctx.fillStyle = '#ffffff';
                ctx.shadowColor = r.color;
                ctx.shadowBlur = 14;
                ctx.beginPath();
                ctx.arc(r.x, r.y, 2.8, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();

                const distToTarget = Math.hypot(r.targetX - r.x, r.targetY - r.y);
                if (distToTarget < 18 || r.y <= r.targetY) {
                    explodeRocket(r.x, r.y, r.palette);
                    rockets.splice(i, 1);
                }
            }

            for (let i = particles.length - 1; i >= 0; i--) {
                const p = particles[i];
                p.x += p.vx;
                p.y += p.vy;
                p.vx *= 0.965;
                p.vy = p.vy * 0.965 + 0.08 * cfg.speed;
                p.alpha -= p.decay;

                if (p.alpha <= 0) {
                    particles.splice(i, 1);
                    continue;
                }

                const currentAlpha = p.flicker && Math.random() < 0.3 ? p.alpha * 0.3 : p.alpha;

                ctx.save();
                ctx.fillStyle = p.color;
                ctx.globalAlpha = currentAlpha;
                ctx.shadowColor = p.color;
                ctx.shadowBlur = p.size > 2.0 ? 12 : 5;
                ctx.beginPath();
                ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
            }

            requestAnimationFrame(animate);
        };

        animate();

        if (cfg.autoDismiss && cfg.duration > 0) {
            setTimeout(() => {
                window.removeEventListener('pointerdown', onPointerDown);
                window.removeEventListener('pointermove', onPointerMove);
                window.removeEventListener('pointerup', onPointerUp);
                window.removeEventListener('touchstart', onPointerDown);
                window.removeEventListener('touchmove', onPointerMove);
                window.removeEventListener('touchend', onPointerUp);
                window.removeEventListener('resize', resize);
                fadeOutAndRemove(600);
            }, Math.max(4500, cfg.duration));
        }

        const observer = new MutationObserver(() => {
            if (!document.body.contains(container)) {
                window.removeEventListener('pointerdown', onPointerDown);
                window.removeEventListener('pointermove', onPointerMove);
                window.removeEventListener('pointerup', onPointerUp);
                window.removeEventListener('touchstart', onPointerDown);
                window.removeEventListener('touchmove', onPointerMove);
                window.removeEventListener('touchend', onPointerUp);
                window.removeEventListener('resize', resize);
                observer.disconnect();
            }
        });
        observer.observe(document.body, { childList: true });

        return;
    } else if (effect === 'retro-pong') {
        container.style.background = '#060812';
        document.body.style.overflow = 'hidden';
        document.body.style.touchAction = 'none';

        const canvas = document.createElement('canvas');
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        container.appendChild(canvas);
        const ctx = canvas.getContext('2d');

        let paddleWidth, paddleHeight, ballRadius;
        const computeDimensions = () => {
            canvas.width = window.innerWidth;
            canvas.height = window.innerHeight;
            paddleWidth = Math.max(14, Math.min(22, canvas.width * 0.016));
            paddleHeight = Math.max(85, Math.min(140, canvas.height * 0.18));
            ballRadius = Math.max(7, Math.min(13, canvas.width * 0.009));
        };
        computeDimensions();

        window.addEventListener('resize', computeDimensions);

        let playerY = (canvas.height - paddleHeight) / 2;
        let targetPlayerY = playerY;
        let aiY = (canvas.height - paddleHeight) / 2;
        let aiTargetY = aiY;
        let aiAimBias = 0;

        let ball = {
            x: canvas.width / 2,
            y: canvas.height / 2,
            dx: 0,
            dy: 0,
            speed: 0,
            trail: []
        };

        let playerScore = 0;
        let aiScore = 0;
        let gameState = 'serve';
        let serveTimer = 45;
        let server = 'player';
        let sparks = [];
        let screenShake = 0;

        const maxScore = 3;

        const spawnSparks = (x, y, color, count = 12) => {
            for (let i = 0; i < count; i++) {
                const angle = Math.random() * Math.PI * 2;
                const spd = Math.random() * 5 + 2;
                sparks.push({
                    x, y,
                    vx: Math.cos(angle) * spd,
                    vy: Math.sin(angle) * spd,
                    color: color,
                    alpha: 1,
                    size: Math.random() * 3 + 1.5
                });
            }
        };

        const resetBall = (winner) => {
            gameState = 'serve';
            serveTimer = 45;
            server = winner === 'ai' ? 'player' : 'ai';
            ball.x = canvas.width / 2;
            ball.y = canvas.height / 2;
            ball.dx = 0;
            ball.dy = 0;
            ball.trail = [];
            ball.speed = Math.max(7.0, Math.min(11.0, canvas.width * 0.007)) * cfg.speed;
            aiAimBias = (Math.random() - 0.5) * (paddleHeight * 0.5);
            aiTargetY = (canvas.height - paddleHeight) / 2;
        };

        const launchBall = () => {
            gameState = 'playing';
            const dirX = server === 'player' ? 1 : -1;
            const angle = (Math.random() * 0.35 + 0.15) * (Math.random() < 0.5 ? 1 : -1);
            ball.dx = dirX * ball.speed * Math.cos(angle);
            ball.dy = ball.speed * Math.sin(angle);
        };

        resetBall('player');

        const isCloseButtonArea = (x, y) => x > window.innerWidth - 75 && y < 75;

        const onPointerMove = (e) => {
            const p = e.touches && e.touches.length > 0 ? e.touches[0] : e;
            if (isCloseButtonArea(p.clientX, p.clientY)) return;
            targetPlayerY = p.clientY - paddleHeight / 2;
        };

        window.addEventListener('pointermove', onPointerMove);
        window.addEventListener('touchmove', onPointerMove, { passive: false });
        window.addEventListener('touchstart', onPointerMove, { passive: false });
        window.addEventListener('pointerdown', onPointerMove);

        const maxSpeed = Math.max(16, Math.min(30, canvas.width * 0.02)) * cfg.speed;

        const updateGame = () => {
            targetPlayerY = Math.max(10, Math.min(canvas.height - paddleHeight - 10, targetPlayerY));
            playerY += (targetPlayerY - playerY) * 0.45;

            if (gameState === 'playing') {
                const aiPaddleX = canvas.width - 25 - paddleWidth;

                if (ball.dx > 0) {
                    if (ball.x < canvas.width * 0.38) {
                        const centerPos = (canvas.height - paddleHeight) / 2;
                        const relaxedTarget = centerPos * 0.75 + (ball.y - paddleHeight / 2) * 0.25;
                        aiTargetY += (relaxedTarget - aiTargetY) * 0.08;
                    } else {
                        const distToAI = Math.max(1, aiPaddleX - ball.x);
                        const lookAhead = Math.min(28, distToAI / Math.max(0.1, ball.dx));
                        let projectedY = ball.y + ball.dy * lookAhead;
                        projectedY = Math.max(ballRadius, Math.min(canvas.height - ballRadius, projectedY));

                        const targetPos = projectedY - paddleHeight / 2 + aiAimBias;
                        aiTargetY += (targetPos - aiTargetY) * 0.16;
                    }
                } else {
                    const centerPos = (canvas.height - paddleHeight) / 2;
                    aiTargetY += (centerPos - aiTargetY) * 0.06;
                }

                const aiSpeedLimit = Math.max(5.2, Math.min(13.5, ball.speed * 0.92)) * cfg.speed;
                const diff = aiTargetY - aiY;
                if (Math.abs(diff) > 1.2) {
                    aiY += Math.sign(diff) * Math.min(Math.abs(diff), aiSpeedLimit);
                }
            }

            aiY = Math.max(10, Math.min(canvas.height - paddleHeight - 10, aiY));

            if (gameState === 'serve') {
                serveTimer--;
                if (serveTimer <= 0) {
                    launchBall();
                }
                return;
            }

            if (gameState !== 'playing') return;

            ball.trail.push({ x: ball.x, y: ball.y });
            if (ball.trail.length > 7) ball.trail.shift();

            ball.x += ball.dx;
            ball.y += ball.dy;

            if (ball.y - ballRadius <= 0) {
                ball.y = ballRadius;
                ball.dy = Math.abs(ball.dy);
                spawnSparks(ball.x, 0, '#38bdf8', 6);
            } else if (ball.y + ballRadius >= canvas.height) {
                ball.y = canvas.height - ballRadius;
                ball.dy = -Math.abs(ball.dy);
                spawnSparks(ball.x, canvas.height, '#38bdf8', 6);
            }

            const playerPaddleX = 25;
            if (ball.dx < 0 &&
                ball.x - ballRadius <= playerPaddleX + paddleWidth &&
                ball.x + ballRadius >= playerPaddleX &&
                ball.y + ballRadius >= playerY &&
                ball.y - ballRadius <= playerY + paddleHeight) {

                const hitPoint = ball.y - (playerY + paddleHeight / 2);
                const normalizedHit = Math.max(-1, Math.min(1, hitPoint / (paddleHeight / 2)));
                const maxAngle = Math.PI * 0.32;
                const bounceAngle = normalizedHit * maxAngle;

                ball.speed = Math.min(maxSpeed, ball.speed * 1.04 + 0.2);
                ball.dx = ball.speed * Math.cos(bounceAngle);
                ball.dy = ball.speed * Math.sin(bounceAngle);
                ball.x = playerPaddleX + paddleWidth + ballRadius + 1;

                spawnSparks(ball.x, ball.y, '#10b981', 14);
                aiAimBias = (Math.random() - 0.5) * (paddleHeight * 0.45);
                if (navigator.vibrate) navigator.vibrate(12);
            }

            const aiPaddleX = canvas.width - 25 - paddleWidth;
            if (ball.dx > 0 &&
                ball.x + ballRadius >= aiPaddleX &&
                ball.x - ballRadius <= aiPaddleX + paddleWidth &&
                ball.y + ballRadius >= aiY &&
                ball.y - ballRadius <= aiY + paddleHeight) {

                const hitPoint = ball.y - (aiY + paddleHeight / 2);
                const normalizedHit = Math.max(-1, Math.min(1, hitPoint / (paddleHeight / 2)));
                const maxAngle = Math.PI * 0.32;
                const bounceAngle = normalizedHit * maxAngle;

                ball.speed = Math.min(maxSpeed, ball.speed * 1.04 + 0.2);
                ball.dx = -ball.speed * Math.cos(bounceAngle);
                ball.dy = ball.speed * Math.sin(bounceAngle);
                ball.x = aiPaddleX - ballRadius - 1;

                spawnSparks(ball.x, ball.y, '#f43f5e', 14);
            }

            if (ball.x < -ballRadius * 2) {
                aiScore++;
                screenShake = 12;
                if (navigator.vibrate) navigator.vibrate([20, 40, 20]);
                if (aiScore >= maxScore) {
                    gameState = 'gameover';
                } else {
                    resetBall('ai');
                }
            } else if (ball.x > canvas.width + ballRadius * 2) {
                playerScore++;
                screenShake = 12;
                if (navigator.vibrate) navigator.vibrate([30, 50, 30]);
                if (playerScore >= maxScore) {
                    gameState = 'gameover';
                } else {
                    resetBall('player');
                }
            }
        };

        const renderGame = () => {
            if (!document.body.contains(container)) return;

            updateGame();

            ctx.save();
            if (screenShake > 0) {
                ctx.translate((Math.random() - 0.5) * screenShake, (Math.random() - 0.5) * screenShake);
                screenShake *= 0.88;
                if (screenShake < 0.5) screenShake = 0;
            }

            ctx.fillStyle = '#060812';
            ctx.fillRect(0, 0, canvas.width, canvas.height);

            const gridStep = 40;
            ctx.strokeStyle = 'rgba(255, 255, 255, 0.02)';
            ctx.lineWidth = 1;
            for (let x = 0; x < canvas.width; x += gridStep) {
                ctx.beginPath();
                ctx.moveTo(x, 0);
                ctx.lineTo(x, canvas.height);
                ctx.stroke();
            }
            for (let y = 0; y < canvas.height; y += gridStep) {
                ctx.beginPath();
                ctx.moveTo(0, y);
                ctx.lineTo(canvas.width, y);
                ctx.stroke();
            }

            ctx.strokeStyle = 'rgba(255, 255, 255, 0.12)';
            ctx.lineWidth = 3;
            ctx.setLineDash([12, 14]);
            ctx.beginPath();
            ctx.moveTo(canvas.width / 2, 0);
            ctx.lineTo(canvas.width / 2, canvas.height);
            ctx.stroke();
            ctx.setLineDash([]);

            ctx.font = 'bold 80px "JetBrains Mono", monospace';
            ctx.fillStyle = 'rgba(16, 185, 129, 0.35)';
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(playerScore, canvas.width / 4, 90);

            ctx.fillStyle = 'rgba(244, 63, 94, 0.35)';
            ctx.fillText(aiScore, (canvas.width * 3) / 4, 90);

            ctx.font = 'bold 12px monospace';
            ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
            ctx.fillText('FIRST TO 3 WINS // DRAG TO MOVE', canvas.width / 2, 35);

            if (gameState === 'serve') {
                ctx.font = 'bold 18px monospace';
                ctx.fillStyle = server === 'player' ? '#10b981' : '#f43f5e';
                ctx.fillText(server === 'player' ? 'PLAYER SERVE' : 'AI SERVE', canvas.width / 2, canvas.height / 2 - 50);
                const dots = Math.ceil(serveTimer / 15);
                ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
                ctx.fillText('• '.repeat(dots), canvas.width / 2, canvas.height / 2 + 50);
            }

            for (let i = 0; i < ball.trail.length; i++) {
                const t = ball.trail[i];
                const alpha = ((i + 1) / ball.trail.length) * 0.35;
                ctx.beginPath();
                ctx.fillStyle = `rgba(255, 255, 255, ${alpha})`;
                ctx.arc(t.x, t.y, ballRadius * 0.8, 0, Math.PI * 2);
                ctx.fill();
            }

            if (gameState !== 'gameover') {
                ctx.save();
                ctx.shadowColor = '#ffffff';
                ctx.shadowBlur = 15;
                ctx.fillStyle = '#ffffff';
                ctx.beginPath();
                ctx.arc(ball.x, ball.y, ballRadius, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
            }

            ctx.save();
            ctx.shadowColor = '#10b981';
            ctx.shadowBlur = 18;
            ctx.fillStyle = '#10b981';
            ctx.beginPath();
            const pr = 6;
            ctx.moveTo(25 + pr, playerY);
            ctx.arcTo(25 + paddleWidth, playerY, 25 + paddleWidth, playerY + paddleHeight, pr);
            ctx.arcTo(25 + paddleWidth, playerY + paddleHeight, 25, playerY + paddleHeight, pr);
            ctx.arcTo(25, playerY + paddleHeight, 25, playerY, pr);
            ctx.arcTo(25, playerY, 25 + paddleWidth, playerY, pr);
            ctx.closePath();
            ctx.fill();
            ctx.restore();

            ctx.save();
            ctx.shadowColor = '#f43f5e';
            ctx.shadowBlur = 18;
            ctx.fillStyle = '#f43f5e';
            ctx.beginPath();
            const aiPx = canvas.width - 25 - paddleWidth;
            ctx.moveTo(aiPx + pr, aiY);
            ctx.arcTo(aiPx + paddleWidth, aiY, aiPx + paddleWidth, aiY + paddleHeight, pr);
            ctx.arcTo(aiPx + paddleWidth, aiY + paddleHeight, aiPx, aiY + paddleHeight, pr);
            ctx.arcTo(aiPx, aiY + paddleHeight, aiPx, aiY, pr);
            ctx.arcTo(aiPx, aiY, aiPx + paddleWidth, aiY, pr);
            ctx.closePath();
            ctx.fill();
            ctx.restore();

            for (let i = sparks.length - 1; i >= 0; i--) {
                const sp = sparks[i];
                sp.x += sp.vx;
                sp.y += sp.vy;
                sp.vx *= 0.96;
                sp.vy *= 0.96;
                sp.alpha -= 0.035;
                if (sp.alpha <= 0) {
                    sparks.splice(i, 1);
                    continue;
                }
                ctx.save();
                ctx.globalAlpha = sp.alpha;
                ctx.fillStyle = sp.color;
                ctx.beginPath();
                ctx.arc(sp.x, sp.y, sp.size, 0, Math.PI * 2);
                ctx.fill();
                ctx.restore();
            }

            if (gameState === 'gameover') {
                ctx.fillStyle = 'rgba(0, 0, 0, 0.75)';
                ctx.fillRect(0, 0, canvas.width, canvas.height);

                const playerWon = playerScore >= maxScore;
                ctx.font = 'bold 56px monospace';
                ctx.fillStyle = playerWon ? '#10b981' : '#f43f5e';
                ctx.shadowColor = playerWon ? '#10b981' : '#f43f5e';
                ctx.shadowBlur = 25;
                ctx.fillText(playerWon ? 'VICTORY' : 'DEFEAT', canvas.width / 2, canvas.height / 2 - 20);

                ctx.font = '24px monospace';
                ctx.shadowBlur = 0;
                ctx.fillStyle = '#e2e8f0';
                ctx.fillText(`${playerScore} - ${aiScore}`, canvas.width / 2, canvas.height / 2 + 45);

                if (!window.pongEnding) {
                    window.pongEnding = true;
                    setTimeout(() => {
                        window.removeEventListener('resize', computeDimensions);
                        window.removeEventListener('pointermove', onPointerMove);
                        window.removeEventListener('touchmove', onPointerMove);
                        window.removeEventListener('touchstart', onPointerMove);
                        window.removeEventListener('pointerdown', onPointerMove);
                        fadeOutAndRemove(0);
                        window.pongEnding = false;
                    }, 2800);
                }
            }

            ctx.restore();

            requestAnimationFrame(renderGame);
        };

        renderGame();

        const observer = new MutationObserver(() => {
            if (!document.body.contains(container)) {
                window.removeEventListener('resize', computeDimensions);
                window.removeEventListener('pointermove', onPointerMove);
                window.removeEventListener('touchmove', onPointerMove);
                window.removeEventListener('touchstart', onPointerMove);
                window.removeEventListener('pointerdown', onPointerMove);
                observer.disconnect();
            }
        });
        observer.observe(document.body, { childList: true });

        return;
    }
}
